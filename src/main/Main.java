...
public class Main {
	...
	double sd_time = Math
		.sqrt(timeList.stream().mapToDouble(e -> (e - ave_time) * (e - ave_time)).average().getAsDouble());
	double sd_length = Math.sqrt(
		pathLengthList.stream().mapToDouble(e -> (e - ave_length) * (e - ave_length)).average().getAsDouble());
