...
public class CitiesData {
    ...
    private static HashSet<Integer>[] withinKNeighbor;
	...
	withinKNeighbor = new HashSet[noOfCities];
	...
	// }
	// Arrays.sort(data, (a, b) -> ((Double) a.dist).compareTo((Double)
	// b.dist));
	// for (int j = 1; j < 51; j++) {
	...
	// k近傍リストの作成
	for (int i = 0; i < noOfCities; i++) {
	    HashSet<Integer> kNeighbor = new HashSet<>();
	    for (int j = 0; j < 50; j++) {
		kNeighbor.addAll(getRevNeighbor(j, i));
	    }
	    setKNeighbor(i, kNeighbor);
	}
    }
    ...
    private static HashSet<Integer> getRevNeighbor(int k, int city) {
    ...
    public static HashSet<Integer> getWithinKNeighbor(int city) {
	return withinKNeighbor[city];
    }

    private void setKNeighbor(int city, HashSet<Integer> hashList) {
	withinKNeighbor[city] = hashList;
    }

    public static void main(String[] args) {
	CitiesData data = new CitiesData("qa194", 194);
	data.outputFile("neighbor_qa194_1.csv");
    }
