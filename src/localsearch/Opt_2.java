...
public class Opt_2 {
	...
	while (true) {
		...
		for (int j = 0; j < K; j++) {
		    ...
		    // d
		    if (CitiesData.calcDistance(cityA, cityC)
			    + CitiesData.calcDistance(cityB, cityD) < CitiesData.calcDistance(cityA, cityB)
				    + CitiesData.calcDistance(cityC, cityD)) {
			path.filpPath(cityA, cityB, cityC, cityD, i);
			...
			HashSet<Integer> candA = (HashSet<Integer>) CitiesData.getWithinKNeighbor(cityA).clone();
